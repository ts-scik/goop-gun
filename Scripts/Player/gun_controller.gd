...
func _manage_gun_unaimed(delta) -> void:
	# get target pos/rot
	...
	if(aim_held or ads_timer > 0.0):
		...
		position = lerp(unaimed_target_pos, last_aimed_target_pos, ads_ratio())
		rotation = lerp(unaimed_target_rot, last_aimed_target_rot, ads_ratio())
	else:
...
func _manage_gun_aimed():
	...
	rotation = Basis.looking_at(fw_dir, Vector3.UP, false).get_euler()
